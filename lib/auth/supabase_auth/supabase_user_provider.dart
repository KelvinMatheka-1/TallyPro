...
class TallyProSupabaseUser extends BaseAuthUser {
  TallyProSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> tallyProSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TallyProSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
